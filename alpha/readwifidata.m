...
[fileName, filePath] = uigetfile(format,'导入外部数据','*.txt','MultiSelect','off');
...
[pathstr, name, ext] = fileparts(fileFullName);
fidin = fopen(fileFullName, 'r');
...
rawdata_wifi{2, k} = [arss / a, brss / b, crss / c];
n=1;idx = 0;
for j = 1:size(rawdata_wifi,2)
    if any(isnan(rawdata_wifi{2,j})) ~= 0
        idx(n) = j;
        n = n + 1;
    end
end
